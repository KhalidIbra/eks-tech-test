...
.
├── terraform/
│   ├── environments/dev/      # composition: backend, providers, module wiring
│   └── modules/
│       ├── networking/         # VPC, subnets, NAT, route tables
│       ├── eks/                # cluster, node group, IRSA roles, OIDC
│       ├── rds/                # MySQL Multi-AZ, subnet group, SG, Secrets Manager
│       ├── ecr/                # immutable repo, lifecycle policy, scan-on-push
│       └── dns-certs/          # ACM cert, Route53 records, DNS validation
├── k8s/
│   ├── platform/
│   │   └── aws-load-balancer-controller/   # upstream manifest + IRSA patch
│   └── apps/
│       └── webserver/          # Deployment, Service, Ingress, Kustomization
├── argocd/
│   ├── root-app.yaml           # bootstrap: app of apps
│   └── apps/
│       ├── aws-load-balancer-controller.yaml
│       ├── monitoring.yaml     # kube-prometheus-stack via ArgoCD/Helm
│       └── webserver.yaml
├── app/                        # Flask webserver
├── .github/workflows/
│   ├── build.yml               # lint, test, scan, push to ECR
│   └── gitops-deploy.yml       # updates manifest in Git, triggers ArgoCD sync
└── scripts/
    └── bootstrap.sh            # one-time post-Terraform cluster bootstrap
```
...
**Kustomize for the app** This is a simple app in a single environment so I chose Kustomize for simplicity and easy integration with kubectl instead of HELM charts.
